...
// database is let instead of const so that I can to modify it in test.js
let database = {
  users: {},
  articles: {},
  nextArticleId: 1,
...
function updateComments(url, request) {
  ...
  if (!requestComments || !requestComments.body) {
    response.status = 400;
  } else if (!savedComments) {
    response.status = 404;
  } else {
    savedComments.body = requestComments.body;
...
function deleteComments(url, request) {
  ...
  if (savedComments) {
    ...
    const articleCommentIds = database.articles[savedComments.articleId].commentIds;
    articleCommentIds.splice(articleCommentIds.indexOf(id), 1);
    const userCommentIds = database.users[savedComments.username].commentIds;
    userCommentIds.splice(userCommentIds.indexOf(id), 1);
    response.status = 204;
  } else {
    response.status = 404;
...
// Helpler functions are above this line.
